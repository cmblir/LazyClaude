...
사용량 탭 최상단의 **오늘 실시간** 위젯: 오늘 총/입력/출력/캐시 토큰, 시간대별 차트, 모델별 분포, 분당 burn rate — 탭이 열려 있는 동안 5초마다 갱신.

내부적으로 백그라운드 tail 파서가 `~/.claude/projects/**/*.jsonl` 을 감시하며 새로 추가된 줄만 파싱(파일별 바이트 커서)하므로 서버 재시작 없이 오늘 수치가 최신으로 유지되고, 토큰은 실제 사용된 날짜에 귀속된다 — 자정을 넘긴 세션이 전부 시작일로 집계되는 문제가 사라진다. 서브에이전트/워크플로 transcript 도 포함되며, content block 마다 반복 기록되는 동일 usage 는 `message.id` 기준으로 1회만 집계된다.
...
이력 참고: 업그레이드 후 첫 실행 시 최근 48시간 내 수정된 세션 파일만 턴 단위 이벤트로 백필되고, 더 오래된 이력은 현재 상태부터 추적을 시작한다. 이벤트는 90일 보존 후 자동 정리.
